...
// The variables below store the randomPizzas and their length on the page loads.
...
};  // end function resizePizzas(size)
...
function updatePositions() {
  ...
  // Precalculate the phases for background pizzas since document.body.scrollTop is the same for all pizzas per scroll.
  var phases = [];
  for(var i = 0; i < 5; i++) {    // Only need to calculate 5 phase values
    phases.push(Math.sin((document.body.scrollTop/1250) + i));
  }

  // fill in all the pizza items with the phase values.
  for (i = 0; i < lengthOfItems; i++) {
    items[i].style.left = items[i].basicLeft + 100 * phases[i % 5] + 'px';
    items[i].style.willchange = "translate";
    items[i].style.webkitTransform = "translateZ(0px)";
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // generate only enough pizzas needed instead of fixed amount (200) of pizzas
  ...
  for (var i = 0; i < totalPizzas; i++) {
    ...
    // give the browser a hint to optimize its painting
